class Cafe {
  ...
  getOrder(orderObj) {
    ...
    return `${orderObj.quantity} euros, please.`;
  }
  ...
  processOrder() {
    ...
    let ordered = this.order.shift();
    if (ordered.name.isWarm === true) {
      return `${ordered.quantity} ${ordered.name.name} ready. Be careful, it's hot.`;
    } else {
      return `${ordered.quantity} ${ordered.name.name} ready.`;
    }
  }
}
